class Boid {
  ...
  isAlive = true;
    ...
    const obstaclesAvoidance = this.avoidObstacles(predators);
    const searchFood = this.searchFood(food);
		this.acceleration
			.add(flockForce)
			.add(edges)
  ...
  searchFood (foodArray) {
    const visibleFood = foodArray.filter(f => this.position.dist(f.position) < this.perception);

    let takeFood = createVector(0, 0);
    if (visibleFood.length) {
      // find nearest
      let distance = this.perception;
      let nearest;
      for (let food of visibleFood) {
        const foodDistance = this.position.dist(food.position);
      ...
      if (distance <= 5) {
        // eat food
        const index = foodArray.indexOf(nearest);
        foodArray.splice(index, 1)
      } else {
        takeFood = p5.Vector.sub(nearest.position, this.position);
      }
		}
